...
def test_init():
    ...
    test = lambda z: FullCI(nelec, H, z)
    ...
    test = lambda z: FullCI(nelec, (H, H), z)
    assert_raises(TypeError, test, G)

    test = lambda z: FullCI(nelec, H, G, nuc_nuc=z)
    assert_raises(TypeError, test, "qq")

    test = lambda z: FullCI(nelec, H, G, dtype=z)
    assert_raises(TypeError, test, str)
    assert_raises(TypeError, test, 1.0)

    test = lambda z: FullCI(nelec, H, G, x=z)
...
def test_solve_eigh():
    ...
    geminal = FullCI(nelec, H, G, nuc_nuc=nuc_nuc)
    geminal()
    assert geminal.compute_energy() < e_hf
