...
def find_num_trans(jumbled_set, ordered_set=None):
    """ Finds the number of transpositions necessary to order an arbitrary order
    of creators (or annihilators) into a strictly increasing order.

    Parameters
    ----------
    jumbled_set : tuple, iterable
        Nonordered set of indices of the creators
    ordered_set : tuple, iterable
        Ordered set of indices of the creators
    ...
    Note
    ----
    Basically, we count the number of times each creator needs to hop over another
    to sort it. If the indices of the creators to its left are greater than its
    indext, then it hops over that creator. Once the creator with the lowest index
    hops over all creators with a lower index to its left, the creator with the
    second lowest index hops, and so on, until the creators are ordered from
...
def find_num_trans_dumb(jumbled_set, ordered_set=None):
    """ Finds the number of transpositions necessary to order an arbitrary order
    of creators (or annihilators) into a strictly increasing order.
