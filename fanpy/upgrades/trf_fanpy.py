...
from scipy.optimize._lsq.common import (
    ...
    evaluate_quadratic, right_multiplied_operator, compute_grad, compute_jac_scale,
    update_tr_radius, scale_for_robust_loss_function, print_header_nonlinear,
    print_iteration_nonlinear, find_active_constraints, make_strictly_feasible,
    regularized_lsq_operator, CL_scaling_vector, check_termination
)
...
def trf_bounds_fanpy(fun, jac, x0, f0, J0, lb, ub, ftol, xtol, gtol, max_nfev,
    ...
    schrodinger = tr_options.pop('schrodinger')
    if (
    ...
    if (
        ...
        ham = None
    cost_tol = tr_options.pop('cost_tol', 1e-10)
    ...
    while True:
        ...
        while actual_reduction <= 0 and nfev < max_nfev:
            ...
            # NOTE: not necessary?
            # if wfn:
            #     x_new[param_indices] = wfn.params.flatten()[wfn_indices]
        ...
        # Increment counter for weight adaptation
        if hasattr(schrodinger, 'counter'):
            schrodinger.counter += 1
...
def trf_no_bounds_fanpy(fun, jac, x0, f0, J0, ftol, xtol, gtol, max_nfev,
                        x_scale, loss_function, tr_solver, tr_options, verbose):
    schrodinger = tr_options.pop('schrodinger')
    if (
        schrodinger.wfn in schrodinger.indices_component_params and
...
class ProjectedSchrodingerSystem(ProjectedSchrodinger):
    def __init__(self, objective, variable_energy=False):
        super().__init__(
            objective.wfn,
            objective.ham,
        ...
        if variable_energy:
            self.energy_type = 'variable'
            if objective.energy_type == 'variable':
                self.energy = objective.energy
                self.indices_component_params = objective.indices_component_params
            else:
                self.indices_component_params[self.energy] = [True]
                for i in self.constraints:
                    if isinstance(i, EnergyConstraint):
                        i.indices_component_params = self.indices_component_params
                        i.energy_variable = self.energy

    def objective(self, params):
        return super().objective(
            params, return_energy=False, assign=True, normalize=False, save=False
        )

    def jacobian(self, params):
        return super().jacobian(
            params, return_d_energy=False, assign=False, normalize=False, save=True
        )
...
def least_squares(
        objective, energy_bounds=(-np.inf, np.inf), param_bounds=(-np.inf, np.inf), save_file="",
        variable_energy=False, **kwargs
):
    from scipy.optimize import least_squares as solver
    objective = ProjectedSchrodingerSystem(objective, variable_energy=variable_energy)
...
scipy.optimize._lsq.trf.trf_bounds = trf_bounds_fanpy
scipy.optimize._lsq.trf.trf_no_bounds = trf_no_bounds_fanpy
fanpy.solver.system.least_squares = least_squares
TERMINATION_MESSAGES[5] = (
    "`cost_tol` termination condition is satisfied."
)
