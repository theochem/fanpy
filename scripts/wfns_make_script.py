...
def make_script(nelec, nspin, one_int_file, two_int_file, wfn_type, nuc_nuc=None,
    ...
    check_inputs(nelec, nspin, one_int_file, two_int_file, wfn_type, pspace_exc, objective, solver,
                 nuc_nuc, optimize_orbs=optimize_orbs,
                 load_orbs=load_orbs, load_ham=load_ham, load_wfn=load_wfn, load_chk=load_chk,
                 save_orbs=save_orbs, save_ham=save_ham, save_wfn=save_wfn, save_chk=save_chk,
                 filename=filename, memory=memory)
    ...
    if solver == 'cma':
        ...
        if solver_kwargs is None:
            solver_kwargs = ("xtol=1.0e-15, ftol=1.0e-15, gtol=1.0e-15, "
    ...
    pspace2 = ('nelec, nspin//2, num_limit=None, exc_orders={}, spin=None, '
               'seniority=wfn.seniority)\n'.format(pspace))
    output += '\n'.join(textwrap.wrap(pspace1 + pspace2,
                                      width=100, subsequent_indent=' '*len(pspace1)))
    output += '\n'
    ...
    if objective == 'system':
        ...
        objective2 = ("wfn, ham, param_selection=param_selection, "
        ...
        objective2 = ("wfn, ham, param_selection=param_selection, "
                      ...
                      "".format(save_chk))
    output += '\n'.join(textwrap.wrap(objective1 + objective2,
                                      width=100, subsequent_indent=' '*len(objective1)))
    output += '\n\n'
    ...
    if solver_name == 'brute':
        ...
        output += '\n'.join(textwrap.wrap(results1 + results2,
                                          width=100, subsequent_indent=' '*len(results1)))
        output += '\n'
    output += '\n'
