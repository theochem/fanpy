...
`Fanpy` is a free and open-source Python 3 library for developing and testing multideterminant
wavefunctions and related *ab initio* methods in electronic structure theory. The main use of
`Fanpy` is to quickly prototype new methods by decreasing the barrier required to translate the
mathematical conception of a new wavefunction ans\"{a}tze to a working implementation. `Fanpy` uses
...
determinant. `Fanpy` is modular both in implementation and in theory. Each electronic structure
...
to quickly try new ideas. `Fanpy` is written purely in Python with standard dependencies, making it
