...
def test_assign_params():
    ...
    indices = [
        ...
        test.get_col_ind((2, 3)),
        test.get_col_ind((5, 6, 7)),
        test.get_col_ind((4,)),
    ]
...
def test_compute_permsum():
    ...
    answer = (
        0 * (7 * 14 * 21 - 7 * 15 * 20 - 8 * 13 * 21 + 8 * 15 * 19 + 9 * 13 * 20 - 9 * 14 * 19)
    ...
    answer = (
        (0 * 7 * 14 + 0 * 8 * 13 + 1 * 6 * 14 + 1 * 8 * 12 + 2 * 6 * 13 + 2 * 7 * 12) * 21
...
def test_olp():
    ...
    test.assign_orbsubsets([(0, 1), (2, 3), (4,), (5, 6, 7), (1, 3, 5), (2, 4, 6, 8), ()])
    test.params = np.arange(24).reshape(4, 6)
    assert np.allclose(test._olp(0b11111111), -test.compute_permsum(2, [0, 1, 3, 2], deriv=None))
