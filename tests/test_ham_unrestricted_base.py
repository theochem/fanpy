...
def test_assign_integrals():
    ...
    with pytest.raises(TypeError):
        BaseUnrestrictedHamiltonian.assign_integrals(
            test, [one_int, one_int.astype(int)] * 2, [two_int] * 3
        )
    with pytest.raises(TypeError):
        BaseUnrestrictedHamiltonian.assign_integrals(
            test, [one_int.tolist(), one_int] * 2, [two_int] * 3
        )
    ...
    with pytest.raises(ValueError):
        BaseUnrestrictedHamiltonian.assign_integrals(
            test, [one_int, one_int.reshape(1, 4, 4)], [two_int] * 3
        )
    with pytest.raises(ValueError):
    ...
    with pytest.raises(ValueError):
        BaseUnrestrictedHamiltonian.assign_integrals(
            test, [one_int] * 2, [two_int, two_int, two_int.reshape(1, 4, 4, 4, 4)]
        )
    with pytest.raises(ValueError):
        BaseUnrestrictedHamiltonian.assign_integrals(
            test, [one_int] * 2, [two_int, two_int, two_int[:, :, :, :3]]
        )

    with pytest.raises(ValueError):
        BaseUnrestrictedHamiltonian.assign_integrals(
            test, [one_int[:3, :3], one_int], [two_int] * 3
        )

    with pytest.raises(ValueError):
        BaseUnrestrictedHamiltonian.assign_integrals(
            test, [one_int] * 2, [two_int, two_int, two_int[:3, :3, :3, :3]]
        )

    with pytest.raises(ValueError):
        BaseUnrestrictedHamiltonian.assign_integrals(
            test, [one_int] * 2, [two_int[:3, :3, :3, :3]] * 3
        )
...
def test_orb_rotate_jacobi():
    ...
    for p, q in it.combinations(range(4), 2):
        ...
        ham = Test(
            [np.copy(one_int_alpha), np.copy(one_int_beta)],
            [np.copy(two_int_aaaa), np.copy(two_int_abab), np.copy(two_int_bbbb)],
        )
...
def test_orb_rotate_matrix():
    ...
    ham = disable_abstract(BaseUnrestrictedHamiltonian)(
        (one_int, one_int), (two_int, two_int, two_int)
    )
    ...
    assert np.allclose(
        ham.two_int[0],
        np.einsum(
            "ijkl,ia,jb,kc,ld->abcd", two_orig, transform1, transform1, transform1, transform1
        ),
    )
    assert np.allclose(
        ham.two_int[1],
    ...
    assert np.allclose(
        ham.two_int[1],
        np.einsum(
            "ijkl,ia,jb,kc,ld->abcd", two_orig, transform1, transform2, transform1, transform2
        ),
    )
    assert np.allclose(
        ham.two_int[2],
        np.einsum(
            "ijkl,ia,jb,kc,ld->abcd", two_orig, transform2, transform2, transform2, transform2
        ),
    )
