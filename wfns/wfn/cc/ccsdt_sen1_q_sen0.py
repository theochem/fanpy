...
class CCSDTsen1Qsen0(CCSDTQsen0):
    r"""Coupled Cluster SD with seniority 1 Triples and seniority 0 Quadruples.

    .. math::

        \left| {{\Psi }_{CCSDTQsen0}} \right\rangle =\prod\limits_{i}{\left( 1+\sum\nolimits_{a}
    ...
    def assign_exops(self, indices=None):
        ...
        if indices is not None:
            ...
            for occ_alpha1 in ex_from[:len(ex_from) // 2]:
                for occ2 in [oorb for oorb in ex_from[ex_from.index(occ_alpha1) + 1:]
                             if oorb is not occ_alpha1 + self.nspatial]:
                    for virt_alpha1 in ex_to[:len(ex_to) // 2]:
                        for virt2 in [vorb for vorb in ex_to[ex_to.index(virt_alpha1) + 1:]
                                      if vorb is not virt_alpha1 + self.nspatial]:
